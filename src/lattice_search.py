...
omnidirectional close packing (see ``omni_numbering``) grouped by
frequency shell, with all heavy work vectorized in NumPy (no scipy).

Geometry
--------
Under ``quadray.DEFAULT_EMBEDDING`` the four basis columns satisfy
...
``quadray.distance``.  Every site ``s`` on frequency shell ``g`` of the
...
from omni_numbering import MAX_SHELL, Quadray, _build_through_shell
