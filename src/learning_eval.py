...
the learners of :mod:`ivm_field` and :mod:`ivm_dynamics`:
...
- :func:`cross_validate_field` — k-fold cross-validation of the
  Laplacian-regularized field learner (:meth:`ivm_field.IVMField.learn`):
...
- :func:`trajectory_train_test` — temporal (first-frac) train/test split
  ...
  :func:`ivm_dynamics.fit_trajectory`, scored by multi-step continuation
...
from ivm_dynamics import DynamicsParams, IVMLattice, fit_trajectory, simulate, step
from ivm_field import IVMField, quadray_shell_norm
