...
def test_build_api_index_includes_public_symbols():
    src_dir = os.path.join(os.path.dirname(__file__), "..", "..", "..", "src", "quadmath")
    ...
    assert ("core.quadray", "Quadray") in names
    assert ("inference.information", "free_energy") in names
    assert ("core.linalg_utils", "bareiss_determinant_int") in names
