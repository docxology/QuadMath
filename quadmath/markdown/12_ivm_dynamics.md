...
The heat (diffusion) update blends a field \(u_t \in \mathbb{R}^{N}\) with its
symmetric-normalized adjacency average \(S = D^{-1/2} A D^{-1/2}\) (rows of
...
with identity rows at isolated sites and round-half-to-even ties (numpy
`rint`), computed by `majority_step`. Every new value is a rounded convex
...
minimizes the trajectory mean squared error
...
![**Dynamics and learning on the IVM lattice (\(R=3\) ball, 27 sites)**. Top row: heat-field snapshots at \(t = 0, 20, 40\) (\(\alpha = 0.35\), seed 7) over the embedded quadray sites; color encodes field value on a shared symmetric scale. Bottom left: sum-of-squares histories — the heat curve decreases monotonically (lemma \eqref{eq:ivmdyn-lemma}), the majority curve stays within its initial range. Bottom middle: the `fit_trajectory` MSE landscape over the coupling grid with the identified best \(\alpha\) and the true generating \(\alpha = 0.3\) marked. Bottom right: the final integer majority field.](../output/figures/ivm_dynamics_demo.png)
